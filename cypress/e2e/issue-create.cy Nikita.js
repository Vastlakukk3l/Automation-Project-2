...
describe('Issue create', () => {
  ...
  it('Should create an issue with random data and validate it successfully', () => {
    ...
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      ...
      // Open issue type dropdown and choose Story to enable selecting Task
      cy.get('[data-testid="select:type"]').click();
      cy.get('[data-testid="select-option:Story"]').wait(1000).trigger('mouseover').trigger('click');
      cy.get('[data-testid="icon:story"]').should('be.visible');

      // Now open issue type dropdown and choose Task
      cy.get('[data-testid="select:type"]').click();
      cy.get('[data-testid="select-option:Task"]').wait(1000).trigger('mouseover').trigger('click');
    ...
    cy.get('[data-testid="board-list:backlog"]')
      ...
      .and('have.length', '1')
      .within(() => {
        // Assert that this list contains the new issue with the random title
        cy.get('[data-testid="list-issue"]')
